...
RUN bash -l -c "spack -C /home/runner install --deprecated llvm+link_llvm_dylib+clang+lld${compiler_version} target=x86_64 || true"
RUN bash -l -c "spack -C /home/runner install --deprecated llvm+link_llvm_dylib+clang+lld${compiler_version} target=x86_64"
